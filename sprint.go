...
func (s *SprintService) GetIssuesForSprintWithContext(ctx context.Context, sprintID int, opts *IssueListOptions) ([]Issue, *Response, error) {
	apiEndpoint := fmt.Sprintf("rest/agile/1.0/sprint/%d/issue", sprintID)
...
func (s *SprintService) GetIssuesForSprint(sprintID int) ([]Issue, *Response, error) {
	return s.GetIssuesForSprintWithContext(context.Background(), sprintID, nil)
}
